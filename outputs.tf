...
output "mysql_secret_arn" {
  ...
  value       = aws_secretsmanager_secret.mysql_credentials_v7.arn
}
...
output "mysql_connection_instructions" {
  ...
  value       = <<-EOT
    ...
    Password: Retrieve from AWS Secrets Manager (ARN: ${aws_secretsmanager_secret.mysql_credentials_v7.arn})
  EOT
}
